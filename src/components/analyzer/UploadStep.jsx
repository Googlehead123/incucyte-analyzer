...
const UploadStep = ({ fileInputRef, handleFileUpload, styles }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0' }}>
      {/* A real button, not a clickable div: the file input is display:none, so a
          keyboard-only user previously had nothing to focus and could not upload. */}
      <button type="button" onClick={() => fileInputRef.current?.click()}
        aria-label="Upload Incucyte data file. Supports .txt and .csv exports."
        style={{ width: '100%', maxWidth: '500px', padding: '48px', border: '2px dashed #475569', borderRadius: '16px', backgroundColor: 'rgba(30, 41, 59, 0.3)', cursor: 'pointer', textAlign: 'center', color: 'inherit', font: 'inherit' }}>
      ...
      <input ref={fileInputRef} type="file" accept=".txt,.csv,.tsv" onChange={handleFileUpload} style={{ display: 'none' }} tabIndex={-1} />
    </div>
  );
};
